...
const   max = 256;                                        //the field is 512x512
...
procedure partout(var part,size:integer;list:twod);
var     i,j,s,e:integer;
begin
   s:=((part-1)*size)+1;
   if s < col then s:=col;
   e:=part*size;
   if e>endcol then e:=endcol;
   writeln('s=',s,'e',e);
   begin
   write('    ');
   j:=s;
...
begin
   while size=0 do
   begin
     writeln('How many simbols fit on your screen horizontaly?');
     parse(size,1,max);
   end;
   writeln;
   if endcol < size then ///////
