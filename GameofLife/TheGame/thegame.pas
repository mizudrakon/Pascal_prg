...
var     field : twod;
        stop,next:boolean;                 //stop is used to stop the program by appropriate input, next for adding new patterns later
...
begin
   ...
   begin
     ...
     begin
        ...
        m:=10*m+n;
        inc(i);
        if (m<min) or (a<m) then    //also the number must not be greater then specified ceiling
        begin
           writeln('Wrong input! Must be integer from {',min,',...,',a,'}bleh');
           m:=s;
           break;
        end;
     end;
